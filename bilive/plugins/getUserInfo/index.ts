...
class GetUserInfo extends Plugin {
  ...
  // 原始消息数据
  private rawMsg: any = {}
  // 欲Log之消息(non Markdown)
  private logMsg: string = ''
  // 欲sendSCMSG之消息(with Markdown)
  private pushMsg: string = ''
  /**
   * 用户信息
   *
   * @private
  ...
  private async _getUserInfo(users: Map<string, User>) {
    for (const [uid, user] of users) {
      ...
      if (getUserInfo === undefined || getUserInfo.response.statusCode !== 200) return
      if (getUserInfo.body.code === 'REPONSE_OK') tmp['liveData'] = getUserInfo.body.data
      else tmp['liveData'] = false
      let medalNum = 0
      ...
      if (getMedalInfo === undefined) return
      else {
        if (getMedalInfo.response.statusCode === 200 && getMedalInfo.body.code === 0) {
          const medalData = getMedalInfo.body.data
          if (medalData.count === 0) tmp['medalData'] = 0
          else {
            medalData.fansMedalList.forEach(medal => {
              if (medal.status === 1) tmp['medalData'] = medal
              else medalNum++
            })
            if (medalNum === medalData.count) tmp['medalData'] = -1
          }
        }
        else tmp['medalData'] = false
      }
      ...
      if (bagInfo === undefined || bagInfo.response.statusCode !== 200 || bagInfo.body.code !== 0) tmp['bagData'] = false
      else if (bagInfo.body.data.length === 0) tmp['bagData'] = 0
      else tmp['bagData'] = bagInfo.body.data
      this.rawMsg[uid] = tmp
    }
    this._logMSGHandler(this.rawMsg)
    this._pushMSGHandler(this.rawMsg)
  }
  /**
   * 处理logMSG
   *
  ...
  private _logMSGHandler(rawMsg: any) {
    for (const uid in rawMsg) {
      let line, live, medal, bag: string = ''
      let user = rawMsg[uid]
...
银瓜子：${user.liveData.silver}  硬币：${user.liveData.billCoin}`)
        }
      }()
      medal = function() {
    ...
    for (const uid in rawMsg) {
      ...
      live = function() {
        if (user.liveData === false) return (`## 用户信息获取失败\n`)
          ...
          return (`## 用户信息\n- ID：${user.liveData.uname} LV${user.liveData.user_level}\n
